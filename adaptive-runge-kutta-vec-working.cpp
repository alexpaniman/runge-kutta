...
point runge_kutta_step(auto &&f, point nth, double step_size, dmatrix &a, dvector &b, dvector &c) {
    ...
    for (size_t i = 0; i < num_stages; ++ i) {
        ...
        double k_i = f(nth.t + c[i] * step_size, f_y);
        sum += b[i] * k_i;
    }
...
std::vector<point> runge_kutta(auto &&f, point init,
                               double step_size, double t_end,
                               dmatrix &a, dvector &b, dvector &c,
                               int num_points_to_save) {
    ...
    std::vector<point> y(num_steps / saved_point_frequency);
    int point_index = 0;
...
int main() {
    ...
    auto f = [](auto t, auto y) { return sin(t); };

    std::ios::sync_with_stdio(0);
    std::cout.tie(0); std::cin.tie(0);

    const int points_to_plot = 2000;
    auto points = runge_kutta(f, { 0, -1, }, .00001, 1000, a, b, c, points_to_plot);

    for (auto &&point: points)
        std::cout << point.t << " " << point.y << "\n";
}
